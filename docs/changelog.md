...
- `pyodide.loadPackage`, `pyodide.runPythonAsync` and
  `pyodide.loadPackagesFromImport` no longer take `messageCallback` and
  `errorCallback` as arguments. Instead, the messages are only logged via
  `console.log` and `console.error` (which is done regardless previously), and
  the promise is rejected when a fatal error occurs.
