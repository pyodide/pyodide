...
$(CPYTHONLIB)/tzdata :
	python3 -m pip install tzdata --target=$(CPYTHONLIB)
...
build/pyodide.asm.js: \
   ...
   # Note(@gzuidhof): -i.bak instead of -i necessary for Mac compatibility,
   # see https://stackoverflow.com/questions/5694228/sed-in-place-flag-that-works-both-on-mac-bsd-and-linux
	sed -i.bak -E 's/var __Z[^;]*;//g' build/pyodide.asm.js
   # Note(@gzuidhof): for mac compatibility we can't use i\ flag, so we use this construction to prepend text
   # see https://stackoverflow.com/questions/40477948/prepend-a-text-to-a-file-in-makefile
   # newlines unfortunately don't work :(
	sed -i.bak '1{h;s/.*/"use strict";\
