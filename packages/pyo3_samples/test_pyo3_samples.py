...
def test_cffi_asprintf():
    ...
    assert pyo3_samples.count_occurences("the skunk saw the people eat the the.", "the") == 4
    ...
    assert pyo3_samples.human_says_hi('{"name" : "Joshua", "age" : 11, "address" : "22nd Place"}') == "Joshua"
    with raises(ValueError, "Failed to parse human_data: missing field `age` at line 1 column 19"):
        assert pyo3_samples.human_says_hi('{"name" : "albert"}')
    with raises(ValueError, "Failed to parse human_data: trailing comma at line 1 column 20"):
        assert pyo3_samples.human_says_hi('{"name" : "albert",}')
