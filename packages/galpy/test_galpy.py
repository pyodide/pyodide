...
DECORATORS = [
    pytest.mark.xfail_browsers(node="galpy loads matplotlib and there are no supported matplotlib backends on node"),
...
def test_actionAngle(selenium):
    ...
    assert numpy.fabs(numpy.std(jrs)/numpy.mean(jrs)) < 1e-4, 'Actions not conserved during orbit integration'
    assert numpy.fabs(numpy.std(jzs)/numpy.mean(jzs)) < 1e-3, 'Actions not conserved during orbit integration'
    return None
