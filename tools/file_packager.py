...
if len(sys.argv) == 1:
    print(
        """Usage: file_packager.py TARGET [--preload A...] [--embed B...] [--exclude C...] [--no-closure]] [--js-output=OUTPUT.js] [--no-force] [--use-preload-cache] [--no-heap-copy] [--separate-metadata]
...
def has_hidden_attribute(filepath):
    ...
    try:
        attrs = ctypes.windll.kernel32.GetFileAttributesW(u"%s" % filepath)
        assert attrs != -1
        result = bool(attrs & 2)
    except Exception:
        result = False
