...
EM_JS_RET_REF(JsRef, hiwire_incref, (JsRef idval), {
  // clang-format off
  if (idval % 2 === 0) {
    // clang-format on
    return;
...
EM_JS_RET_INT(void, hiwire_decref, (JsRef idval), {
  Module.hiwire.decref(idval);
});

EM_JS_RET_REF(JsRef, hiwire_int, (int val), {
  return Module.hiwire.new_value(val);
});

EM_JS_RET_REF(JsRef, hiwire_double, (double val), {
  return Module.hiwire.new_value(val);
});

EM_JS_RET_REF(JsRef, hiwire_string_ucs4, (const char* ptr, int len), {
  var jsstr = "";
  var idx = ptr / 4;
  for (var i = 0; i < len; ++i) {
...
EM_JS_RET_REF(JsRef, hiwire_string_ucs2, (const char* ptr, int len), {
  var jsstr = "";
  var idx = ptr / 2;
  for (var i = 0; i < len; ++i) {
...
EM_JS_RET_REF(JsRef, hiwire_string_ucs1, (const char* ptr, int len), {
  var jsstr = "";
  var idx = ptr;
  for (var i = 0; i < len; ++i) {
...
EM_JS_RET_REF(JsRef, hiwire_string_utf8, (const char* ptr), {
  return Module.hiwire.new_value(UTF8ToString(ptr));
});

EM_JS_RET_REF(JsRef, hiwire_string_ascii, (const char* ptr), {
  return Module.hiwire.new_value(AsciiToString(ptr));
});

EM_JS_RET_REF(JsRef, hiwire_bytes, (char* ptr, int len), {
  var bytes = new Uint8ClampedArray(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(bytes);
});

EM_JS_RET_REF(JsRef, hiwire_int8array, (i8 * ptr, int len), {
  var array = new Int8Array(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(array);
})

EM_JS_RET_REF(JsRef, hiwire_uint8array, (u8 * ptr, int len), {
  var array = new Uint8Array(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(array);
})

EM_JS_RET_REF(JsRef, hiwire_int16array, (i16 * ptr, int len), {
  var array = new Int16Array(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(array);
})

EM_JS_RET_REF(JsRef, hiwire_uint16array, (u16 * ptr, int len), {
  var array = new Uint16Array(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(array);
})

EM_JS_RET_REF(JsRef, hiwire_int32array, (i32 * ptr, int len), {
  var array = new Int32Array(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(array);
})

EM_JS_RET_REF(JsRef, hiwire_uint32array, (u32 * ptr, int len), {
  var array = new Uint32Array(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(array);
})

EM_JS_RET_REF(JsRef, hiwire_float32array, (f32 * ptr, int len), {
  var array = new Float32Array(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(array);
})

EM_JS_RET_REF(JsRef, hiwire_float64array, (f64 * ptr, int len), {
  var array = new Float64Array(Module.HEAPU8.buffer, ptr, len);
  return Module.hiwire.new_value(array);
})

EM_JS_RET_INT(void, hiwire_throw_error, (JsRef idmsg), {
  var jsmsg = Module.hiwire.get_value(idmsg);
  throw new Error(jsmsg);
});

EM_JS_RET_REF(JsRef, hiwire_array, (), { return Module.hiwire.new_value([]); });

EM_JS_RET_INT(void, hiwire_push_array, (JsRef idarr, JsRef idval), {
  Module.hiwire.get_value(idarr).push(Module.hiwire.get_value(idval));
});

EM_JS_RET_REF(JsRef, hiwire_object, (), {
...
EM_JS_RET_REF(JsRef, hiwire_get_global, (const char* ptrname), {
...
EM_JS_RET_REF(JsRef, hiwire_get_member_int, (JsRef idobj, int idx), {
  var jsobj = Module.hiwire.get_value(idobj);
...
EM_JS_RET_INT(void,
              ...
              (JsRef idobj, int idx, JsRef idval),
              {
                Module.hiwire.get_value(idobj)[idx] =
                  Module.hiwire.get_value(idval);
              });

EM_JS_RET_REF(JsRef, hiwire_get_member_obj, (JsRef idobj, JsRef ididx), {
  var jsobj = Module.hiwire.get_value(idobj);
...
EM_JS_RET_INT(void, hiwire_delete_member_obj, (JsRef idobj, JsRef ididx), {
  var jsobj = Module.hiwire.get_value(idobj);
  var jsidx = Module.hiwire.get_value(ididx);
  delete jsobj[jsidx];
});

EM_JS_RET_REF(JsRef, hiwire_dir, (JsRef idobj), {
  var jsobj = Module.hiwire.get_value(idobj);
  var result = [];
  do {
...
EM_JS_RET_REF(JsRef, hiwire_call, (JsRef idfunc, JsRef idargs), {
...
EM_JS_RET_REF(JsRef,
              ...
              (JsRef idobj, const char* ptrname, JsRef idargs),
              {
                ...
                var jsargs = Module.hiwire.get_value(idargs);
                return Module.hiwire.new_value(
                  jsobj[jsname].apply(jsobj, jsargs));
              });

EM_JS_RET_REF(JsRef, hiwire_new, (JsRef idobj, JsRef idargs), {
...
EM_JS_RET_INT(int, hiwire_get_length, (JsRef idobj), {
  return Module.hiwire.get_value(idobj).length;
});

EM_JS_RET_INT(bool, hiwire_get_bool, (JsRef idobj), {
...
EM_JS_RET_REF(JsRef, hiwire_next, (JsRef idobj), {
  // clang-format off
...
EM_JS_RET_REF(JsRef, hiwire_get_iterator, (JsRef idobj), {
  // clang-format off
  if (idobj === Module.hiwire.UNDEFINED) {
    return Module.hiwire.ERROR;
  }
...
EM_JS_RET_INT(bool, hiwire_nonzero, (JsRef idobj), {
  var jsobj = Module.hiwire.get_value(idobj);
  // TODO: should this be !== 0?
  return (jsobj != 0) ? 1 : 0;
});

EM_JS_RET_INT(bool, hiwire_is_typedarray, (JsRef idobj), {
  var jsobj = Module.hiwire.get_value(idobj);
  // clang-format off
...
EM_JS_RET_INT(bool, hiwire_is_on_wasm_heap, (JsRef idobj), {
  var jsobj = Module.hiwire.get_value(idobj);
  // clang-format off
...
EM_JS_RET_INT(int, hiwire_get_byteOffset, (JsRef idobj), {
  var jsobj = Module.hiwire.get_value(idobj);
  return jsobj['byteOffset'];
});

EM_JS_RET_INT(int, hiwire_get_byteLength, (JsRef idobj), {
  var jsobj = Module.hiwire.get_value(idobj);
  return jsobj['byteLength'];
});

EM_JS_RET_INT(void, hiwire_copy_to_ptr, (JsRef idobj, void* ptr), {
  var jsobj = Module.hiwire.get_value(idobj);
  // clang-format off
...
EM_JS_RET_INT(void,
              ...
              (JsRef idobj, char** format_ptr, Py_ssize_t* size_ptr),
              {
                ...
                let[format_utf8, size] =
                  Module.hiwire.dtype_map.get(jsobj.constructor.name) ||
                  [ 0, 0 ];
...
EM_JS_RET_REF(JsRef, hiwire_subarray, (JsRef idarr, int start, int end), {
