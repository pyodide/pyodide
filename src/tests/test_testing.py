...
def test_C_test_entrypoints(selenium):
    assert selenium.run_js(
        "return pyodide._module.Tests.test_entrypoints() === 'It works!';"
    )
...
def test_C_tests_succeed1(selenium):
    assert_C_test(
        selenium.run_js(
            "return pyodide._module.Tests.c_tests_expect_success_success();"
        )
    )
...
def test_C_tests_succeed2(selenium):
    assert_C_test(
        selenium.run_js("return pyodide._module.Tests.c_tests_expect_fail_fail();")
    )
...
def test_C_tests_fail1(selenium):
    ...
    with pytest.raises(AssertionError, match=msg):
        assert_C_test(
            selenium.run_js(
                "return pyodide._module.Tests.c_tests_expect_success_fails();"
            )
        )
...
def test_C_tests_fail2(selenium):
    ...
    with pytest.raises(AssertionError, match=msg):
        assert_C_test(
            selenium.run_js(
                "return pyodide._module.Tests.c_tests_expect_fail_succeeds();"
            )
        )
...
def test_C_tests_fail3(selenium):
    ...
    with pytest.raises(AssertionError, match=msg):
        assert_C_test(
            selenium.run_js(
                "return pyodide._module.Tests.c_tests_expect_fail_wrong_message();"
            )
        )
