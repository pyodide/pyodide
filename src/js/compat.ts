...
): { response: Promise<Response>; binary?: undefined } | { binary: Promise<Uint8Array> } {
      ...
      if (path.includes("://")) {
            ...
            return {
                  binary: nodeFsPromisesMod
                        .readFile(path)
                        .then((data: Buffer) => new Uint8Array(data.buffer, data.byteOffset, data.byteLength)),
            };
      }
}
...
) => { response: Promise<Response>; binary?: undefined } | { response?: undefined; binary: Promise<Uint8Array> };
if (IN_NODE) {
...
export async function loadBinaryFile(path: string, file_sub_resource_hash?: string | undefined): Promise<Uint8Array> {
...
export function calculateInstallBaseUrl(lockFileURL: string) {
      ...
      return lockFileURL.substring(0, lockFileURL.lastIndexOf("/") + 1) || globalThis.location?.toString() || ".";
}
