...
/** @hidden */
export type InFuncType = () => null | undefined | string | ArrayBuffer | Uint8Array | number;
...
function readWriteHelper(stream: Stream, cb: () => number, method: string) {
      ...
      if (nbytes === undefined) {
            ...
            console.warn(`${method} returned undefined; a correct implementation must return a number`);
            throw new FS.ErrnoError(cDefs.EIO);
      }
      if (nbytes !== 0) {
...
const prepareBuffer = (buffer: Uint8Array, offset: number, length: number): Uint8Array =>
 ...
 * @private
 */
API.initializeStreams = function (stdin?: InFuncType, stdout?: (a: string) => void, stderr?: (a: string) => void) {
...
export function setStdin(
      ...
      let { stdin, error, isatty, autoEOF, read } = options as StdinOptions & Partial<Reader>;
      const numset = +!!stdin + +!!error + +!!read;
      if (numset > 1) {
            throw new TypeError("At most one of stdin, read, and error must be provided.");
      }
      if (!stdin && autoEOF !== undefined) {
            throw new TypeError("The 'autoEOF' option can only be used with the 'stdin' option");
      }
      if (numset === 0) {
            setDefaultStdin();
            return;
...
function _setStdwrite(
      ...
      let { raw, isatty, batched, write } = options as StdwriteOpts & Partial<Writer>;
      ...
      if (nset > 1) {
            throw new TypeError("At most one of 'raw', 'batched', and 'write' must be passed");
      }
      if (!raw && !write && isatty) {
            throw new TypeError("Cannot set 'isatty' to true unless 'raw' or 'write' is provided");
      }
      if (raw) {
      ...
      _getInput(): Uint8Array | string | number | undefined {
            ...
            if (ArrayBuffer.isView(val)) {
                  if ((val as any).BYTES_PER_ELEMENT !== 1) {
                        console.warn(`Expected BYTES_PER_ELEMENT to be 1, infunc gave ${val.constructor}`);
                        throw new FS.ErrnoError(cDefs.EIO);
                  }
                  return val;
            }
            ...
            console.warn("Expected result to be undefined, null, string, array buffer, or array buffer view");
            throw new FS.ErrnoError(cDefs.EIO);
      }
