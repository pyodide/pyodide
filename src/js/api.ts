...
import { makeGlobalsProxy, SnapshotConfig, syncUpSnapshotLoad1, syncUpSnapshotLoad2 } from "./snapshot";
...
export class PyodideAPI_ {
       ...
       * main();
       */
      static runPython(code: string, options: { globals?: PyProxy; locals?: PyProxy; filename?: string } = {}): any {
            if (!options.globals) {
      ...
      static unpackArchive(
            ...
            if (!ArrayBuffer.isView(buffer) && API.getTypeTag(buffer) !== "[object ArrayBuffer]") {
      ...
      static async mountNativeFS(
            ...
            Module.FS.mount(Module.FS.filesystems.NATIVEFS_ASYNC, { fileSystemHandle }, path);
      ...
      static mountNodeFS(emscriptenPath: string, hostPath: string): void {
            ...
            Module.FS.mount(Module.FS.filesystems.NODEFS, { root: hostPath }, emscriptenPath);
      }
...
API.bootstrapFinalizedPromise = new Promise<void>((r) => (bootstrapFinalized = r));
...
API.finalizeBootstrap = function (
      ...
      // Set up globals
      let globals = API.runPythonInternal("import __main__; __main__.__dict__") as PyDict;
      let builtins = API.runPythonInternal("import builtins; builtins.__dict__") as PyDict;
      ...
      API.os.environ.__setitem__("LD_LIBRARY_PATH", API.defaultLdLibraryPath.join(":"));
