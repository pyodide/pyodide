...
function getRunDependencyPromise(num_dependencies = 1) {
  return new Promise((resolve, reject) => {
    ...
    console.log("returning promise for dep idx #", dependencyIndex);
    dependencyCallbacks.set(dependencyIndex, resolve);
  });
}
...
function monitorRunDependencies() {
  ...
  if (resolvedDependencies % 2 === 0) {
    ...
    let callback = dependencyCallbacks.get(resolvedDependencies);
    if (callback) {
      callback();
    }
  }
...
// loading a package
// TODO: attaching a global error handler for this seems like a really bad idea.
// What are we trying to catch? Can't we be a bit more specific?
function windowErrorHandler(err) {
  console.error("deleting monitorRunDependencies");
