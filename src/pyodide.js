...
globalThis.languagePluginLoader = new Promise((resolve, reject) => {
  ...
  Module.postRun = async () => {
    ...
    // lots of things in tests try to wait for processes to end using
    // os.waitpid. This breaks because emscripten doesn't support processes (and
    // in recent emscripten this syscall actually calls abort on the wasm
    // runtime so most tests break.)
    pyodide.runPython(
