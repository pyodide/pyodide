...
JsProxy_SetAttr(PyObject* self, PyObject* attr, PyObject* pyvalue)
{
  ...
  if (pyvalue == NULL) {
    ...
    idvalue = python2js_deep(pyvalue);
    FAIL_IF_MINUS_ONE(
...
JsProxy_Call(PyObject* self, PyObject* args, PyObject* kwargs)
{
  ...
  for (Py_ssize_t i = 0; i < nargs; ++i) {
    idarg = python2js_deep(PyTuple_GET_ITEM(args, i));
    FAIL_IF_NULL(idarg);
  ...
  if (PyDict_Size(kwargs)) {
    idkwargs = python2js_deep(kwargs);
...
JsProxy_RichCompare(PyObject* a, PyObject* b, int op)
{
  ...
  int result;
  JsRef ida = python2js_deep(a);
  JsRef idb = python2js_deep(b);
  switch (op) {
    case Py_LT:
...
JsProxy_New(PyObject* o, PyObject* args, PyObject* kwargs)
{
  ...
  for (Py_ssize_t i = 0; i < nargs; ++i) {
    JsRef idarg = python2js_deep(PyTuple_GET_ITEM(args, i));
...
JsProxy_subscript(PyObject* o, PyObject* pyidx)
{
  ...
  JsRef ididx = python2js_deep(pyidx);
