...
async function handleWatImports(input) {
    ...
    return input.replaceAll(import_re, function(_, name, file) {
        ...
        const wasmModule = parseWat(name, readFileSync(join(__dirname, file), {encoding: "utf8"}));
...
async function buildContinuations() {
    const input = readFileSync(join(__dirname, "./continuations.js"), {encoding: "utf8"});
    ...
    writeFileSync(join(__dirname, "./continuations.gen.js.c"), emjs_wrapped_output.join("\n"));
}
